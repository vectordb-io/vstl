...
class binarySearchTree : public bsTree<K,E>,
  ...
  public:
    ...
    bool empty() const {
        return linkedBinaryTree<pair<const K, E>>::treeSize == 0;
    }
    int size() const {
        return linkedBinaryTree<pair<const K, E>>::treeSize;
    }
    ...
    void ascend() {
        linkedBinaryTree<pair<const K, E>>::inOrderOutput();
    }
};
...
void binarySearchTree<K,E>::erase(const K& theKey) {
    ...
    if (p->leftChild != NULL && p->rightChild != NULL) {
        ...
        if (pp == NULL)
            linkedBinaryTree<pair<const K, E>>::root = q;
    ...
    // delete p
    if (p == linkedBinaryTree<pair<const K, E>>::root)
        linkedBinaryTree<pair<const K, E>>::root = c;
    else {
