...
// Visit existing pair, if any, with same key.
    ...
    binaryTreeNode<pair<const K, E> > *p = binarySearchTree<K,E>::root, // search pointer
                                       *pp = 0;   // parent of p
    while (p != NULL) {
        // examine p->element
        pp = p;
    ...
    if (binarySearchTree<K,E>::root != NULL) // the tree is not empty
        if (thePair.first < pp->element.first)
            pp->leftChild = newNode;
        else
            pp->rightChild = newNode;
    else
        binarySearchTree<K,E>::root = newNode; // insertion into empty tree
